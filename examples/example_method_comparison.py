...
def run_comparison():
    ...
    dt_implicit = 0.001    # Can be larger
    dt_crank = 0.001       # Can be larger
    ...
    # ========== Explicit Euler ==========
    print("\n[1/3] Running Explicit Euler method...")
    ...
    # ========== Implicit Euler ==========
    print("\n[2/3] Running Implicit Euler method...")
    ...
    # ========== Crank-Nicolson ==========
    print("\n[3/3] Running Crank-Nicolson method...")
    ...
    ax.plot(x, u_explicit, 'b-', linewidth=1.5, label=f'Explicit Euler (dt={dt_explicit})')
    ax.plot(x, u_implicit, 'r-', linewidth=1.5, label=f'Implicit Euler (dt={dt_implicit})')
    ax.plot(x, u_crank, 'g-', linewidth=1.5, label=f'Crank-Nicolson (dt={dt_crank})')
    ...
    # Plot 2: Errors
    ax = axes[0, 1]
    ax.plot(x, u_explicit - u_analytical, 'b-', label='Explicit Euler')
    ax.plot(x, u_implicit - u_analytical, 'r-', label='Implicit Euler')
    ax.plot(x, u_crank - u_analytical, 'g-', label='Crank-Nicolson')
    ...
    ax = axes[1, 0]
    methods = ['Explicit\nEuler', 'Implicit\nEuler', 'Crank-\nNicolson']
    times = [time_explicit, time_implicit, time_crank]
    ...
    ax = axes[1, 1]
    errors = [error_explicit, error_implicit, error_crank]
    ...
    plt.tight_layout()
    plt.savefig('example_method_comparison.png', dpi=150, bbox_inches='tight')
