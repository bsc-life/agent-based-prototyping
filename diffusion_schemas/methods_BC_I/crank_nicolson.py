...
class CrankNicolsonBCISchema(Schema):
    """
    ...
    def _build_system_matrices(self) -> None:
        ...
        if self.ndim == 1:
            self.A_impl = self._build_matrices_1d()
        elif self.ndim == 2:
            self.A_impl = self._build_matrices_2d()
        elif self.ndim == 3:
            self.A_impl = self._build_matrices_3d()
        else:
    ...
    def step(self) -> None:
        ...
        # NOTE: 
        # bulk_rhs_np1 is the bulk contribution at t^(n+1)
        # agent_source_np1 is (should, agents are wip) the agent contribution at t^(n+1)
        # source_n contains agent and bulk contributions at t^n 
        ...
        system_matrix = self.A_impl.tolil(copy=True)
        system_matrix.setdiag(system_matrix.diagonal() + self.theta * self.dt * lhs)
