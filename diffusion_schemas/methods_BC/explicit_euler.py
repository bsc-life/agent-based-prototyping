...
class ExplicitEulerBCSchema(Schema):
    """
    ...
    def step(self) -> None:
        ...
        # Neumann are already handled in the Laplacian computation
    ...
    def _laplacian_1d(self, u: np.ndarray) -> np.ndarray:
        ...
        # These are no flux boundaries (Neumann BC with zero gradient)
        ...
        if self._boundary_conditions is not None:
            ...
            if isinstance(self._boundary_conditions, DirichletBC):
                # For Dirichlet BC, the boundary values are fixed
                # We can set the Laplacian to zero at boundaries
                # and then impose the fixed values after the step
                laplacian[0] = 0
                laplacian[-1] = 0

            if isinstance(self._boundary_conditions, NeumannBC):
                # For Neumann BC, the gradient at the boundary is fixed
                # Assuming same gradient at both boundaries for simplicity
    ...
    def _laplacian_2d(self, u: np.ndarray) -> np.ndarray:
        ...
        # Zero-flux (Neumann) boundaries by default
        ...
        # Apply BC adjustments if necessary
        if self._boundary_conditions is not None:

            if isinstance(self._boundary_conditions, DirichletBC):
                # For Dirichlet, we set laplacian to zero on boundaries
                # (values enforced after step)
                laplacian[0, :] = 0
                laplacian[-1, :] = 0
                laplacian[:, 0] = 0
                laplacian[:, -1] = 0
            ...
            if isinstance(self._boundary_conditions, NeumannBC):
                ...
                # Left and right edges (x-direction, i = 0 and -1)
                laplacian[0, 1:-1] = (
                    2 * (u[1, 1:-1] - u[0, 1:-1]) / (dx**2) - 2 * flux / dx
                    + (u[0, 2:] - 2*u[0, 1:-1] + u[0, :-2]) / (dy**2)
                )
                laplacian[-1, 1:-1] = (
                    2 * (u[-2, 1:-1] - u[-1, 1:-1]) / (dx**2) + 2 * flux / dx
                    + (u[-1, 2:] - 2*u[-1, 1:-1] + u[-1, :-2]) / (dy**2)
                )
                
                # Bottom and top edges (y-direction, j = 0 and -1)
                laplacian[1:-1, 0] = (
                    (u[2:, 0] - 2*u[1:-1, 0] + u[:-2, 0]) / (dx**2)
                ...
                # Corners
                laplacian[0, 0] = (
                    2 * (u[1, 0] - u[0, 0]) / (dx**2) - 2 * flux / dx
                    + 2 * (u[0, 1] - u[0, 0]) / (dy**2) - 2 * flux / dy
                )
                laplacian[0, -1] = (
                    2 * (u[1, -1] - u[0, -1]) / (dx**2) - 2 * flux / dx
                    + 2 * (u[0, -2] - u[0, -1]) / (dy**2) + 2 * flux / dy
                )
                laplacian[-1, 0] = (
                    2 * (u[-2, 0] - u[-1, 0]) / (dx**2) + 2 * flux / dx
                    + 2 * (u[-1, 1] - u[-1, 0]) / (dy**2) - 2 * flux / dy
                )
                laplacian[-1, -1] = (
                    2 * (u[-2, -1] - u[-1, -1]) / (dx**2) + 2 * flux / dx
                    + 2 * (u[-1, -2] - u[-1, -1]) / (dy**2) + 2 * flux / dy
                )
    ...
    def _laplacian_3d(self, u: np.ndarray) -> np.ndarray:
        ...
        if self._boundary_conditions is not None:
            ...
            if isinstance(self._boundary_conditions, NeumannBC):
                ...
                # X-direction boundaries (i = 0 and -1)
                laplacian[0, 1:-1, 1:-1] = (
                ...
                # Y-direction boundaries (j = 0 and -1)
                laplacian[1:-1, 0, 1:-1] = (
                ...
                # Z-direction boundaries (k = 0 and -1)
                laplacian[1:-1, 1:-1, 0] = (
                ...
                # XZ edges
                laplacian[0, 1:-1, 0] = (
                    2 * (u[1, 1:-1, 0] - u[0, 1:-1, 0]) / (dx**2) - 2 * flux / dx
                ...
                laplacian[-1, 1:-1, 0] = (
                    2 * (u[-2, 1:-1, 0] - u[-1, 1:-1, 0]) / (dx**2) + 2 * flux / dx
