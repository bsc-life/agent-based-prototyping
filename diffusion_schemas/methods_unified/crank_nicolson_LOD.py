    ...
    def _step_lod_bci_opt(self, rhs, bulk_lhs_np1):
        ...
        # ==========================================================
            ...
            if (not has_per_node_source) and (not has_dirichlet) and (not is_neumann) and getattr(self, '_lu_x', None) is not None:
                ...
                b_x = (1.0 + 2*alpha_x + decay_term) + (self.theta * self.dt / 2.0) * bulk_lhs_np1  # (Nx, Ny)
            ...
            if (not has_per_node_source) and (not has_dirichlet) and (not is_neumann) and getattr(self, '_lu_y', None) is not None:
                ...
                b_y = (1.0 + 2*alpha_y + decay_term) + (self.theta * self.dt / 2.0) * bulk_lhs_np1.T  # (Ny, Nx)
                ...
                self.state = self._thomas_batched(a_y, b_y, c_y, d_y).T  # transpose back to (Nx, Ny)
        ...
        # ==========================================================
            ...
            if (not has_per_node_source) and (not has_dirichlet) and (not is_neumann) and getattr(self, '_lu_x', None) is not None:
                ...
                a_x = np.full(Nx, -alpha_x); a_x[0] = 0.0
                c_x = np.full(Nx, -alpha_x); c_x[-1] = 0.0
                b_x = (1.0 + 2*alpha_x + decay_term) + (self.theta * self.dt / 3.0) * bulk_lhs_np1.reshape(Nx, Ny*Nz)
                d_x = rhs_3d.reshape(Nx, Ny*Nz).copy()
            ...
            if (not has_per_node_source) and (not has_dirichlet) and (not is_neumann) and getattr(self, '_lu_y', None) is not None:
                ...
                a_y = np.full(Ny, -alpha_y); a_y[0] = 0.0
                c_y = np.full(Ny, -alpha_y); c_y[-1] = 0.0
                b_y = (1.0 + 2*alpha_y + decay_term) + (self.theta * self.dt / 3.0) * bulk_lhs_np1.transpose(1, 0, 2).reshape(Ny, Nx*Nz)
            ...
            if (not has_per_node_source) and (not has_dirichlet) and (not is_neumann) and getattr(self, '_lu_z', None) is not None:
                ...
                b_z = (1.0 + 2*alpha_z + decay_term) + (self.theta * self.dt / 3.0) * bulk_lhs_np1.transpose(2, 0, 1).reshape(Nz, Nx*Ny)
                ...
                self.state = self._thomas_batched(a_z, b_z, c_z, d_z).reshape(Nz, Nx, Ny).transpose(1, 2, 0)
