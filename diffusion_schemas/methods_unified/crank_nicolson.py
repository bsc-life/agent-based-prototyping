...
class _CrankNicolsonUnified(Schema):
    """Crank-Nicolson core with selectable behavior variants."""
    ...
    def _build_system_matrices(self) -> None:
        ...
        if self._variant == "bci":
            if self.ndim == 1:
                self.A_impl = self._build_matrices_1d(implicit_only=True)
            elif self.ndim == 2:
                self.A_impl = self._build_matrices_2d(implicit_only=True)
            elif self.ndim == 3:
    ...
    def step(self) -> None:
        if self._variant == "base":
            ...
            raise ValueError(f"Unsupported Crank-Nicolson variant: {self._variant}")

    def _step_base(self) -> None:
        source_n = self._compute_source_term()
        source_np1 = source_n

        rhs = self.A_expl.dot(self.state.flatten()) + self.dt * self.theta * source_np1.flatten() + self.dt * (1 - self.theta) * source_n.flatten()
    ...
    def set_boundary_conditions(self, boundary_conditions) -> None:
        ...
        if self._variant not in {"bc", "bci", "bcos"}:
            return
...
class CrankNicolsonSchema(_CrankNicolsonUnified):
...
class CrankNicolsonBCSchema(_CrankNicolsonUnified):
    """Crank-Nicolson method with boundary conditions."""
...
class CrankNicolsonBCISchema(_CrankNicolsonUnified):
    """Crank-Nicolson method with implicit sources and BCs."""
...
class CrankNicolsonBCOSSchema(_CrankNicolsonUnified):
    """Crank-Nicolson method with operator-split sources and BCs."""
